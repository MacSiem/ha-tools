...
class HAVacuumWaterMonitor extends HTMLElement {
  ...
  _buildWaterTab(device, data) {
    ...
    return `
      <div class="tab-content">
        ...
        ${this._buildRefillMethodsSection(device)}
  ...
  _buildStatsTab(devices) {
    ...
    return `
      <div class="tab-content">
        ...
        ${(this._userDevices || []).length > 0 ? `<div class="section-block"><div class="section-title">\u2795 Dodane recznie</div>${this._userDevices.map(ud => `<div class="disc-row"><span class="disc-name">${ud.icon || '\uD83E\uDDA4'} ${this._sanitize(ud.name)}</span><span class="disc-id">${ud.vacuum_entity}</span><button class="maint-del-btn user-dev-remove" data-entity="${ud.vacuum_entity}" title="Usun">🗑️</button></div>`).join('')}</div>` : ''}
        ${fullTankTip}
        ${discoveredHtml}
  ...
  --bento-text-muted: var(--vwm-text-muted);
        ...
        /* Tab navigation */
        .tab-nav { display: flex; gap: 2px; margin-bottom: 14px; background: var(--vwm-overlay-light); border-radius: 10px; padding: 3px; }
