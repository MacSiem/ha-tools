...
class HAEnergyEmail extends HTMLElement {
  ...
  _getOverviewDataForPeriod(period) {
    ...
    if (period === 'day') {
      return manual.map(d => ({
        ...
        lastMonth: 0,
        cost: this._float(this._state(d.energy_day || d.energy_week, '0')) * (this._config.energy_price || 0.65),
        source: 'manual'
      })).sort((a, b) => b.month - a.month);
    }
    ...
    for (const dev of this._discoveredDevices) {
      ...
      if (match) {
        result.push({
          ...
          cost: match.value * (this._config.energy_price || 0.65),
    ...
    try {
      ...
      for (const [entityId, dataPoints] of Object.entries(stats || {})) {
        ...
        result.push({
          ...
          cost: kwh * (this._config.energy_price || 0.65),
  ...
  _getOverviewData() {
    ...
    if (this._discoveredDevices && this._discoveredDevices.length > 0) {
      return this._filterExcluded(this._discoveredDevices.map(d => ({
        ...
        month: d.value_kwh,
        lastMonth: 0,
        cost: d.value_kwh * (this._config.energy_price || 0.65),
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .stat { background: var(--bg); border: 1px solid var(--bo); border-radius: var(--r2); padding: 14px; text-align: center; min-width: 0; overflow: hidden; }
        .stat-val { font-size: 24px; font-weight: 700; color: var(--tx); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .empty-state .title { font-size: 15px; font-weight: 600; color: var(--tx); margin-bottom: 6px; }
        .empty-state .desc { font-size: 13px; color: var(--t2); line-height: 1.6; max-width: 400px; margin: 0 auto; }
        ...
        .email-setup { background: var(--wa-l); border: 1px solid rgba(245,158,11,.3); border-radius: var(--r2); padding: 14px; margin-bottom: 16px; }
        .email-setup-title { font-size: 13px; font-weight: 600; color: var(--tx); margin-bottom: 8px; }
        .email-input-row { display: flex; gap: 8px; align-items: center; }
        .email-input { flex: 1; padding: 8px 12px; border: 1.5px solid var(--bo); border-radius: var(--r1); font-size: 13px; font-family: 'Inter', sans-serif; background: var(--ca); color: var(--tx); outline: none; }
        .email-input:focus { border-color: var(--pr); box-shadow: 0 0 0 3px var(--pr-l); }
        .svc-select-btn { transition: all .2s; }
        .svc-select-btn:not(.btn-primary):hover { background: var(--pr-l) !important; }
        .email-bar { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; padding: 8px 12px; background: var(--ok-l); border-radius: var(--r1); font-size: 12px; color: var(--tx); }
        .email-edit-btn { background: none; border: none; color: var(--pr); cursor: pointer; font-size: 11px; padding: 2px 6px; font-family: 'Inter', sans-serif; }
        .email-edit-btn:hover { text-decoration: underline; }
        .source-manual { background: var(--ok-l); color: var(--ok); }
        .config-section { margin-bottom: 20px; }
        .config-section-title { font-size: 13px; font-weight: 700; color: var(--tx); margin-bottom: 10px; display: flex; align-items: center; gap: 6px; }
        .device-toggle { display: flex; align-items: center; gap: 10px; padding: 8px 10px; border-radius: var(--r1); transition: background .15s; }
        .device-toggle:hover { background: var(--pr-l); }
        .device-toggle label { flex: 1; font-size: 13px; color: var(--tx); cursor: pointer; display: flex; align-items: center; gap: 8px; }
        .device-toggle .dt-val { font-size: 11px; color: var(--t2); min-width: 60px; text-align: right; }
        .toggle-switch { position: relative; width: 36px; height: 20px; flex-shrink: 0; }
        .toggle-switch input { opacity: 0; width: 0; height: 0; }
        .toggle-slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background: var(--bo); border-radius: 20px; transition: .2s; }
        ...
        .toggle-switch input:checked + .toggle-slider { background: var(--pr); }
        .toggle-switch input:checked + .toggle-slider::before { transform: translateX(16px); }
        .config-input-row { display: flex; gap: 8px; align-items: center; margin-bottom: 8px; }
        .config-input-row label { font-size: 12px; color: var(--t2); min-width: 80px; font-weight: 500; }
        .config-input { padding: 6px 10px; border: 1.5px solid var(--bo); border-radius: var(--r1); font-size: 13px; background: var(--ca); color: var(--tx); font-family: 'Inter', sans-serif; }
        .config-input:focus { border-color: var(--pr); outline: none; box-shadow: 0 0 0 3px var(--pr-l); }
        .device-count { font-size: 11px; color: var(--t3); font-weight: 400; }
      ...
      <div class="card">
        <div class="header">
          ...
          <div>
            ...
            <div class="header-sub">${recipientDisplay} \u00A0\u2022\u00A0 <span id="price-display" style="cursor:pointer;color:var(--pr);border-bottom:1px dashed var(--pr)" title="${L ? 'Kliknij aby zmieni\u0107' : 'Click to change'}">${this._config.currency} ${this._config.energy_price}/kWh \u270E</span></div>
          </div>
        </div>
        <div class="tabs">
          ...
          <button class="tab ${this._activeTab === 'config' ? 'active' : ''}" data-tab="config">\u2699\uFE0F Config</button>
        </div>
  ...
  _bindPriceEdit() {
    ...
    priceEl.addEventListener('click', (e) => {
      e.preventDefault();
      const L = this._lang === 'pl';
      const cur = this._config.energy_price || 0.65;
      ...
      const inputHtml = `<span style="display:inline-flex;align-items:center;gap:4px">
        ...
        <input type="number" id="price-input" value="${cur}" step="0.01" min="0" style="width:70px;padding:3px 6px;border:1.5px solid var(--pr);border-radius:4px;font-size:12px;background:var(--ca);color:var(--tx);font-family:'Inter',sans-serif;text-align:center">
        <span>/kWh</span>
  ...
  _tabOverview() {
    ...
    if (devData.length === 0 && !this._discoveryDone) {
      return `<div class="empty-state">
        <div class="big"><span class="spinner" style="width:32px;height:32px;border-width:3px;border-color:var(--pr);border-top-color:transparent;"></span></div>
    ...
    const totalCost = isAuto ? totalEnergy * (this._config.energy_price || 0.65) : displayData.reduce((s, d) => s + d.cost, 0);
    ...
    const periodBtns = ['day', 'week', 'month', 'total'].map(p => {
      ...
      return `<button class="overview-period-btn" data-period="${p}" style="padding:5px 12px;font-size:11px;border-radius:6px;cursor:pointer;border:1px solid var(--bo,#e2e8f0);background:${period === p ? 'var(--pr,#3B82F6)' : 'var(--bg,#f8fafc)'};color:${period === p ? '#fff' : 'var(--tx,#1e293b)'};font-weight:${period === p ? '600' : '400'};">${lb}</button>`;
    }).join('');
    return `
      ${isAuto ? `<div class="info-row">\u{1F50D}\u00A0 ${L ? 'Auto-discovery: znaleziono <b>' + displayData.length + '</b> urz\u0105dze\u0144 z czujnikami energii.' : 'Auto-discovery: found <b>' + displayData.length + '</b> devices with energy sensors.'} <span class="source-badge source-auto">AUTO</span>${periodNote ? `<br><span style="font-size:11px;color:var(--wa)">${periodNote}</span>` : ''}</div>` : ''}
      ...
      ${displayData.map(d => {
        ...
        const entityInfo = d.entity_id ? `<span style="font-size:10px;color:var(--t3)" title="${d.entity_id}">${d.entity_id.split('.')[1].substring(0,20)}</span>` : '';
  ...
  _tabSchedule() {
    ...
    const scheduleCard = (icon, nameL, nameE, state, timeLabel, enableId, disableId, createId, timeInputId, timeValue, extraInputHtml) => {
      ...
      return `<div class="schedule-card">
        ...
        <div style="display:flex;gap:8px;align-items:center;margin-top:10px;flex-wrap:wrap">
          <label style="font-size:12px;color:var(--t2);font-weight:500">${L ? 'Godzina' : 'Time'}:</label>
          <input type="time" id="${timeInputId}" value="${timeValue}" style="padding:5px 10px;border:1.5px solid var(--bo);border-radius:var(--r1);font-size:13px;background:var(--ca);color:var(--tx);font-family:'Inter',sans-serif;">
          ${extraInputHtml || ''}
        </div>
        <div class="btn-row">
          ${ex ? (state === 'on'
    ...
    return `
      ...
      ${scheduleCard(
        ...
        `<label style="font-size:12px;color:var(--t2);font-weight:500;margin-left:8px">${L ? 'Dzie\u0144' : 'Day'}:</label>
         <select id="day-weekly" style="padding:5px 10px;border:1.5px solid var(--bo);border-radius:var(--r1);font-size:13px;background:var(--ca);color:var(--tx);font-family:'Inter',sans-serif">${dayOptions}</select>`
      )}
      ${scheduleCard(
  ...
  _tabPreview() {
    ...
    if (!this._discoveryDone) {
      return `<div class="empty-state"><div class="big"><span class="spinner" style="width:32px;height:32px;border-width:3px;border-color:var(--pr);border-top-color:transparent;"></span></div><div class="title">${L ? '\u0141adowanie danych...' : 'Loading data...'}</div></div>`;
    }
    ...
    const getDevData = (period) => {
      if (devices.length > 0) {
        return devices.map(d => {
          ...
          if (period === 'day') { current = this._float(this._state(d.energy_day || d.energy_week, '0')); cost = current * (this._config.energy_price || 0.65); }
      ...
      if (periodData && periodData.length > 0 && periodData.some(d => d.month > 0)) {
        return periodData.map(d => ({ name: d.name, current: d.month, previous: d.lastMonth || 0, cost: d.cost || d.month * (this._config.energy_price || 0.65), hasPeriod: true })).sort((a, b) => b.current - a.current);
      }
      return autoDevices.map(d => ({ name: d.name, current: d.value_kwh, previous: 0, cost: d.value_kwh * (this._config.energy_price || 0.65), hasPeriod: false })).sort((a, b) => b.current - a.current);
    };
    const renderReport = (p) => {
      ...
      const periodNote = !isPeriodData && isAuto ? `<div style="font-size:11px;color:var(--t2);margin-bottom:6px;font-style:italic">\u26A0 ${L ? 'Brak sensor\u00F3w dla tego okresu \u2014 pokazano dane total' : 'No period-specific sensors found \u2014 showing total data'}</div>` : '';
      return `<div class="preview-box" style="margin-bottom:14px">
        <h3 style="margin:0 0 8px">${p.icon} ${title} \u2013 ${today}</h3>
        ${periodNote}
        <div style="font-size:12px;color:var(--t2);margin-bottom:10px">\u{1F4E7} ${recipientLine} \u00A0\u2022\u00A0 ${range} \u00A0\u2022\u00A0 ${devData.length} ${L ? 'urz.' : 'dev.'}</div>
        <div style="display:flex;gap:16px;margin-bottom:10px;flex-wrap:wrap">
          <div><span style="font-size:18px;font-weight:700;color:#F59E0B">${totalEnergy.toFixed(1)}</span> <span style="font-size:11px;color:var(--t2)">kWh</span></div>
          <div><span style="font-size:18px;font-weight:700;color:#3B82F6">${totalCost.toFixed(2)}</span> <span style="font-size:11px;color:var(--t2)">${this._config.currency}</span></div>
        </div>
        <table class="preview-table">
          ...
          ${devData.length > 5 ? `<tr><td colspan="3" style="text-align:center;color:var(--t2);font-size:11px">+ ${devData.length - 5} ${L ? 'wi\u0119cej urz\u0105dze\u0144' : 'more devices'}...</td></tr>` : ''}</tbody>
        </table>
      </div>`;
    };
    return `
      ...
      <div style="font-size:11px;color:var(--t2);margin-top:4px">${L ? 'Podgl\u0105d tre\u015Bci emaila. Rzeczywisty email zawiera pe\u0142n\u0105 tabel\u0119 HTML.' : 'Preview of email content. Actual email contains full HTML table.'}</div>`;
  }
  ...
  _tabConfig() {
    ...
    const recipient = this._getRecipient();
    const service = this._getNotifyService();
    const price = this._config.energy_price || 0.65;
    ...
    return `
      <div class="config-section">
        ...
        <div class="config-input-row">
          ...
          <span style="font-size:12px;color:var(--t2)">${currency}/kWh</span>
      ...
      <div class="config-section">
        ...
        <div style="max-height:350px;overflow-y:auto;border:1px solid var(--bo);border-radius:var(--r2);padding:4px">
          ${devices.map(d => {
          ...
          ${devices.length === 0 ? `<div style="text-align:center;padding:20px;color:var(--t2);font-size:13px">${L ? 'Brak wykrytych urz\u0105dze\u0144' : 'No devices detected'}</div>` : ''}
        </div>
      </div>
  ...
  async _createAutomation(type, update = false) {
    ...
    const emailMsg = [
      ...
      ``,
      `---`,
      `Generated by HA Energy Email card @ ${price} ${currency}/kWh`
    ].join('\\n');
    const action = [{
    ...
    try {
      ...
      const html = `<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0">
        ...
        <div style="padding:20px 28px">
          ...
          <p style="margin:16px 0 0;font-size:11px;color:#94a3b8;text-align:center">@ ${price} ${currency}/kWh \u2022 HA Energy Email Card</p>
        </div>
      </div>`;
    ...
    if (smtp.found && smtp.services.length > 0) {
      ...
      return `<div class="smtp-section">
        ...
        ${hasMultiple ? `<div style="margin-top:12px">
          <label style="font-size:12px;font-weight:600;color:var(--t2);display:block;margin-bottom:4px">${L ? 'Wybierz serwis do wysy\u0142ki' : 'Select notify service'}:</label>
